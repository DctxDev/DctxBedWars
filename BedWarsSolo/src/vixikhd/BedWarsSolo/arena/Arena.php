...
namespace vixikhd\BedWarsSolo\arena;
...
use vixikhd\BedWarsSolo\libs\muqsit\invmenu\inventory\InvMenuInventory; 
...
use fw\Fireworks;
use vixikhd\BedWarsSolo\ServerManager;
use vixikhd\BedWarsSolo\libs\muqsit\invmenu\{
    InvMenu
};
use pocketmine\nbt\tag\CompoundTag;
use pocketmine\nbt\tag\StringTag;
use pocketmine\nbt\tag\ByteArrayTag; 
use vixikhd\BedWarsSolo\libs\muqsit\invmenu\transaction\{DeterministicInvMenuTransaction, InvMenuTransaction, InvMenuTransactionResult};
...
class Arena implements Listener {
    ...
    public function setColorTag($player){
        $color = [
           ...
           "pink" => "§d",
           "gray" => "§8"];
        $player->setDisplayName("" . $color[$this->getTeam($player)] . "" . $player->getDisplayName() . "");
    }
    ...
    public function joinToArena(Player $player) {
        ...
        $player->getInventory()->setItem(0, Item::get(Item::TOTEM, 0, 1)->setCustomName("§aMy Solo Locker §7[use]"));
        $player->getInventory()->setItem(8, Item::get(437, 0, 1)->setCustomName("§dBack to hub §7[use]"));
        $player->setGamemode($player::ADVENTURE);
        $this->setColorTag($player);
        $player->setHealth(20);
        $player->setFood(20);
        $player->setNameTagVisible();
        $this->broadcastMessage("§7{$player->getDisplayName()} joined. §8[".count($this->players)."/{$this->data["slots"]}]");
    }
    ...
    public function disconnectPlayer(Player $player, string $quitMsg = "", bool $death = false) {
        ...
        if($this->phase == self::PHASE_GAME){
            ...
            if($count <= 0){
                ...
                $this->broadcastMessage("§l§fTEAM ELIMINATED §r$color[$team] team §fwas §celiminated!");
            }
            if($team == "red"){
    ...
    public function spectator(Player $player) {
        ...
        $player->getInventory()->setItem(8, Item::get(437, 0, 1)->setCustomName("§dBack to hub §7[use]"));
        $player->getInventory()->setItem(7, Item::get(Item::HEART_OF_THE_SEA, 0, 1)->setCustomName("§aNew game §7[use]"));
        $this->setGhost($player, true);
        $this->plugin->addLose($player);
        $this->plugin->addDeath($player);
        $player->getInventory()->setItem(0, Item::get(Item::COMPASS, 0, 1)->setCustomName("§eTeleporter §7[use]"));
    ...
    public function startGame() {
        ...
        $this->broadcastMessage("§l§cBedWars");
        $this->broadcastMessage("§fYour bed grants you infinite respawns. Once it has been destroyed, you will no longer respawn. Defend your Bed, destroy others.");
    ...
    public function destroyAllBeds(){
        foreach(["red", "blue", "yellow", "green", "aqua", "white", "pink", "gray"] as $t){
                ...
                if($t == "red"){
                    foreach($this->redteam as $red){
                        if($red instanceof Player){
                            $red->addTitle("§l§fBED DESTRUCTION ", "§r§fyou will no longer respawn");
                ...
                if($t == "blue"){
                    foreach($this->blueteam as $blue){
                        if($blue instanceof Player){
                            $blue->addTitle("§l§fBED DESTRUCTION ", "§r§fyou will no longer respawn");
                ...
                if($t == "yellow"){
                    foreach($this->yellowteam as $yellow){
                        if($yellow instanceof Player){
                            $yellow->addTitle("§l§cBED DESTRUCTION ", "§r§fyou will no longer respawn");
                ...
                if($t == "green"){
                    foreach($this->greenteam as $green){
                        if($green instanceof Player){
                            $green->addTitle("§l§cBED DESTRUCTION ", "§r§fyou will no longer respawn");
                ...
                if($t == "aqua"){
                    foreach($this->aquateam as $aqua){
                        if($aqua instanceof Player){
                            $aqua->addTitle("§l§cBED DESTRUCTION ", "§r§fyou will no longer respawn");
                ...
                if($t == "white"){
                    foreach($this->whiteteam as $white){
                        if($white instanceof Player){
                            $white->addTitle("§l§cBED DESTRUCTION ", "§r§fyou will no longer respawn");
                ...
                if($t == "pink"){
                    foreach($this->pinkteam as $pink){
                        if($pink instanceof Player){
                            $pink->addTitle("§c§l§cBED DESTRUCTION ", "§r§fyou will no longer respawn");
                ...
                if($t == "gray"){
                    foreach($this->grayteam as $gray){
                        if($gray instanceof Player){
                            $gray->addTitle("§l§cBED DESTRUCTION ", "§r§fyou will no longer respawn");
    ...
    public function destroyTreasure($team, $player = null){
        ...
        $tn = ucwords($team);
        if($player instanceof Player){
            $this->broadcastMessage("§l§fBED DESTRUCTION » §r{$c}{$tn} team §fbed was disminated §r§fby {$player->getDisplayName()}");
            if(isset($this->broken[$player->getId()])){
        ...
        foreach($this->players as $p){
            if($p instanceof Player && $this->getTeam($p) == $team){
                $p->sendTitle("§l§cBED DESTROYED ", "§r§fyou will no longer respawn");
    ...
    public function Wins(string $team) {
        ...
        $this->broadcastMessage("§l§fTEAM WINNER $teamName[$team] are the WINNERS!");  
    ...
    public function onChat(PlayerChatEvent $event){
        ...
        if($f == "!"){
            $msg = str_replace("!", "", $msg);
            if(trim($msg) !== ""){
                $this->broadcastMessage("§6SHOUT §r{$player->getDisplayName()}: §f{$msg}");
            }
        } else {
            $team = $this->getTeam($player);
            foreach($this->players as $pt){
                if($this->getTeam($pt) == $team){
                    $pt->sendMessage("§aTEAM §r{$player->getDisplayName()}: §f{$msg}");
                }
            }
        }
        $event->setCancelled();
    ...
    public function onBreak(BlockBreakEvent $event){
        ...
        if($this->inGame($player)){
            ...
            if($block instanceof Bed){
                ...
                $pink = $this->data["treasure"]["pink"];
                $gray = $this->data["treasure"]["gray"];
                if(in_array(($pos = (new Vector3($block->x, $block->y, $block->z))->__toString()), [$red, $blue, $yellow, $green, $aqua, $white, $pink, $gray])){
                    $team = null;
                    if($pos == $red){
                        $team = "red";
                    }
    ...
    public function onPlace(BlockPlaceEvent $event){
        ...
        if($this->inGame($player)){
            ...
            if($block->getY() > $spawnY){
                ...
                switch($rand){
                case 1:
                    $player->sendMessage("§ccant place block outside play region");
    ...
    public function onInteract(PlayerInteractEvent $event) {
        ...
        if($player->getLevel()->getFolderName() == $this->level->getFolderName()){
            if($this->phase == self::PHASE_LOBBY){
                ...
                if($itemN == "§aMy Locker §7[use]"){
                    $this->plugin->LockerForm($player);
                }
                if($item->getId() == 437){
                    if($this->plugin->config["waterdog"]["enabled"]) {
                        ServerManager::transferPlayer($player, $this->plugin->config["waterdog"]["lobbyServer"]);
            ...
            $player->getCursorInventory()->clearAll();
                ...
                if ($item->equalsExact($player->getInventory()->getItemInHand())) {
                    if (is_null($detected)) {
                        $player->sendMessage("§7Can't find nearest player");
                    } else {
                        $player->sendMessage("§7Set compass to nearest player");
                    }
                    }
                }
                }
    ...
    public function onDamage(EntityDamageEvent $event) {
        ...
        if($this->phase == self::PHASE_GAME){
            if($event instanceof EntityDamageByEntityEvent){
                ...
                if($dmg instanceof Player && $player->getNameTag() == "§l§bITEM SHOP\n§r§eLEFT CLICK"){
                    if($this->inGame($dmg)){
                ...
                if($dmg instanceof Player && $player->getNameTag() == "§l§bTEAM UPGRADE\n§r§eLEFT CLICK"){
                    if($this->inGame($dmg)){
                        if($dmg->distance($player) < 5){ 
            ...
            if($this->phase == self::PHASE_GAME){
                ...
                if(isset($this->plugin->lastDamager[$player->getName()])){
                    ...
                    if($player->getHealth() - $event->getBaseDamage() <= 0){
                        ...
                        $this->broadcastMessage("{$player->getDisplayName()} §cdid an oopsie {$txt}"); 
                        $player->getArmorInventory()->clearAll(); 
                        $player->getInventory()->clearAll();
                        $txt = null;
                        if(!$this->hasTreasure($player)){
    ...
    public function manageShop($player, $inv, $type){
        ...
        if($type == "§fUtility"){
        $inv->setItem(18, Item::get(Item::GOLDEN_APPLE, 0, 1)
        ->setLore(["§63 Gold"])
        ->setCustomName("§aGolden Apple")
        );
        $inv->setItem(19, Item::get(Item::SNOWBALL, 0, 1)
        ->setLore(["§f40 Iron"])
        ->setCustomName("§aBedbug")
        );
        $inv->setItem(20, Item::get(Item::SPAWN_EGG, 14, 1)
        ->setLore(["§f120 Iron"])
        ->setCustomName("§aDream Defender")
        );
        $inv->setItem(21, Item::get(Item::EGG, 0, 1)
        ->setLore(["§f2 Emerald"])
        ->setCustomName("§aBridge Egg")
        );
