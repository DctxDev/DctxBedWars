...
use vixikhd\BedWarsSolo\math\Time;
use pocketmine\entity\object\ItemEntity;
use vixikhd\BedWarsSolo\math\Vector3;
        ...
        switch ($this->plugin->phase) {
            case Arena::PHASE_LOBBY:
                foreach($this->plugin->players as $player){
                    ...
                    $api->setLine($player, 3, "§f: §e{$map}");
                    $api->setLine($player, 4, "§f: §e{$slots}");
                    $api->setLine($player, 5, "   ");
                    $api->setLine($player, 6, "§f Waiting for player..");
                    $api->setLine($player, 7, "§fMode: §eSolo");
                    $api->setLine($player, 8, " ");
                    $api->setLine($player, 9, "§fTeam: {$color[$team]}");
                    $api->setLine($player, 10, "  ");
                    $api->setLine($player, 11, "§eplay.dctxgames.net");
                }
                if(count($this->plugin->players) >= 6) {
                    if($this->waitTime > 0){
                        foreach($this->plugin->players as $player){
                    $team = $this->plugin->getTeam($player); 
                    if(!$player->hasEffect(14)){
            ...
            case Arena::PHASE_GAME:
                ...
                foreach($this->plugin->players as $player){
                    ...
                    $api = Scoreboards::getInstance();
                    $api->new($player, "BedWarsSolo", "§l§eBEDWARS");
                    $api->setLine($player, 1, "§7{$date}"); 
                    $api->setLine($player, 2, " "); 
                    ...
                    $api->setLine($player, 14, "§eplay.dctxgames.net");
                }
            ...
            case Arena::PHASE_RESTART:
                ...
                $this->plugin->broadcastMessage("§bsearcing for new game §f{$this->restartTime}...", Arena::MSG_TIP);
                $this->restartTime--;
